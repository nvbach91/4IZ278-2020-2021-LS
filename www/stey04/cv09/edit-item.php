...
if (@$_GET) {
    if (@$_SESSION['user_id']) {
        ...
        $stmt = $pdo->prepare('SELECT privilege FROM users WHERE id = :id LIMIT 1');
        $stmt->execute(['id' => $user_id]);
        $privilege = (int) $stmt->fetchColumn();
        if ($privilege == 1) {
            exit('You must have 2 or 3 privilege rights to do this!');
        }
    } else {
        exit('Please login or sign up');
    }
