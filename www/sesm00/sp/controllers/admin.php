...
$usersDB = new UsersDB();
$usersDB->verifyUserAccess(PRIVILEGE_MANAGER | PRIVILEGE_ADMINISTRATOR);
...
if (isset($_POST['action'])) {
    if ($_POST['action'] == "editPrepare") {
        ...
        if (count($searchedProducts) > 0) {
            ...
            $locked = false;
            if ($editedProduct['user_edit'] != null) {
                $currUserId = $usersDB->getCurrentUserId();
                $currTime = getCurrentTimestamp();
                if ($editedProduct['user_edit'] != $currUserId && ($editedProduct['last_edit'] + (5 * 60)) >= $currTime) {
                    $locked = true;
                }
            }
            if ($locked) {
                $_POST['action'] = "fail";
                $isEditing = false;
                array_push($messages, array('type' => 'danger', 'text' => "Produkt je editován jiným uživatelem. Současná editace není možná"));
            } else {
                $productsDB->update(array('last_edit' => getCurrentTimestamp(), 'user_edit' => $usersDB->getCurrentUserId()), array('id' => $editedProduct['id']));
                array_push($messages, array('type' => 'warning', 'text' => "Produkt bude k editaci uzamčen po dobu pěti minut"));
            }
        } else {
            $isEditing = false;
    ...
    if ($_POST['action'] == "add" || $_POST['action'] == "edit") {
        ...
        if ($_POST['action'] == "add" && $validForm && $usersDB->hasUserAccess(PRIVILEGE_ADMINISTRATOR)) {
    ...
    if ($_POST['action'] == "delete" && is_numeric($_POST['id']) && $usersDB->hasUserAccess(PRIVILEGE_ADMINISTRATOR)) {
...
<div class="container py-5">
    ...
    <?php if ($usersDB->hasUserAccess(PRIVILEGE_ADMINISTRATOR)) : ?>
    ...
    <?php if ($usersDB->hasUserAccess(PRIVILEGE_ADMINISTRATOR) || ($usersDB->hasUserAccess(PRIVILEGE_MANAGER) && $isEditing)) : ?>
    ...
    <div class="mt-4 vh-100">
        <div class="mh-100 overflow-auto">
            ...
            <?php
                $locked = false;
                if ($product['user_edit'] != null) {
                    $currUserId = $usersDB->getCurrentUserId();
                    $currTime = getCurrentTimestamp();
                    if ($product['user_edit'] != $currUserId && ($product['last_edit'] + (5 * 60)) >= $currTime) {
                        $locked = true;
                    }
                }
            ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 text-right">
                            <form method="post" class="d-inline">
                                ...
                                <?php if ($locked) : ?>
                                    <button class="btn btn-primary disabled" type="button" data-toggle="tooltip" data-placement="right" title="Editace uzamčena jiným uživatelem">Upravit</button>
                                <?php else: ?>
                            ...
                            <form method="post" class="d-inline">
                                ...
                                <input type="hidden" name="id" value="<?php echo $product['id']; ?>">
                                <?php if ($locked) : ?>
                                    <button class="btn btn-danger disabled" type="button" data-toggle="tooltip" data-placement="right" title="Editace uzamčena jiným uživatelem">Smazat</button>
                                <?php else: ?>
